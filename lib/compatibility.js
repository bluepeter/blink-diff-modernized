		...
		generate: function () {
			...
			config = {
				...
				output: {
					...
					composition: options.composition === false ? constants.COMPOSITION_OFF : (options.composeLeftToRight ? constants.COMPOSITION_LEFT_TO_RIGHT : (options.composeTopToBottom ? constants.COMPOSITION_TOP_TO_BOTTOM : constants.COMPOSITION_AUTO)),
					copyImage: (!options.copyImageAToOutput && !options.copyImageBToOutput) ? constants.COPY_IMAGE_OFF : (options.copyImageBToOutput ? constants.COPY_IMAGE_B : constants.COPY_IMAGE_A)
				}
			};
